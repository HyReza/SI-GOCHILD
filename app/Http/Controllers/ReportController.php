...
class ReportController extends Controller
{
    ...
    // public function store(Request $request)
    // {
    //     // 1. LOG DATA REQUEST MASUK
    //     Log::info('=== START STORE REPORT ===');
    //     Log::info('Student ID: ' . $request->student_id);
    //     Log::info('Input Lingkar Kepala (Raw): ' . $request->lingkar_kepala);
    //     // Log::info('Semua Data Request:', $request->all()); // Uncomment jika ingin melihat semua data
    ...
    //             'weight' => $request->berat_badan,
    //             'height' => $request->tinggi_badan,
    //             'head_circumference' => $request->lingkar_kepala, // Pastikan ini ada
    ...
    //         // 2. LOG DATA YANG AKAN DISIMPAN
    //         Log::info('Data Mapping ke DB:', $reportData);

    //         // ... (Kode Upload Foto & Tanda Tangan tetap sama) ...
    //         $photoMap = [
    //             'foto_agama' => 'religious_values_photo',
    //             'foto_jatidiri' => 'identity_photo',
    //             'foto_steam' => 'literacy_steam_photo',
    ...
    //         // 3. LOG SETELAH CREATE BERHASIL
    //         Log::info('Report Created Successfully. ID: ' . $report->id);

    //         // ... (Kode Simpan Scores, Theme Notes, Health tetap sama) ...
    //         if ($request->has('scores')) {
    //             foreach ($request->scores as $materialId => $scoreValue) {
    //                 if ($scoreValue) {
    //                     $material = Material::find($materialId);
    ...
    //         DB::commit();
    //         Log::info('=== END STORE REPORT (SUCCESS) ===');
    //         return redirect()->route('reports.history', $request->student_id)->with('success', 'Raport berhasil dibuat.');
    //     } catch (\Exception $e) {
    //         DB::rollBack();

    //         // 4. LOG ERROR JIKA GAGAL
    //         Log::error('!!! ERROR STORE REPORT !!!');
    //         Log::error('Message: ' . $e->getMessage());
    //         Log::error('Line: ' . $e->getLine());
    //         Log::error('File: ' . $e->getFile());
    //         // Log::error('Trace: ' . $e->getTraceAsString()); // Uncomment untuk trace lengkap

    //         return back()->with('error', 'Gagal menyimpan: ' . $e->getMessage() . ' (Cek Logs)')->withInput();
    //     }
    // }
