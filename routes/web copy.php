...
Route::middleware('auth')->group(function () {
    ...
    // 1. Index Utama (List Siswa)
    Route::get('/reports', [ReportController::class, 'index'])->name('reports.index');

    // 2. History Raport (List Raport milik Siswa X)
    Route::get('/reports/{student}/history', [ReportController::class, 'history'])->name('reports.history');

    // 3. Flow Pembuatan Raport
    // Step 1: Pilih Periode (menggantikan stepTwo yang lama)
    Route::get('/reports/{student}/create-period', [ReportController::class, 'selectPeriod'])->name('reports.selectPeriod');

    // Step 2: Form Penilaian (menggantikan create yang lama)
    ...
    // ====================================================
    // [BARU] FITUR PENYELESAIAN & UPLOAD BUKTI FOTO
    // ====================================================

    // 1. Tampilkan Halaman Form Penyelesaian
    Route::get('/orders/{order}/complete', [ServiceOrderController::class, 'completion'])
        ->name('orders.completion');

    // 2. Proses Simpan Catatan & Foto Bukti
    Route::post('/orders/{order}/complete', [ServiceOrderController::class, 'storeCompletion'])
        ->name('orders.store-completion');
