<x-app-layout>
    ...
    <div id="deleteModal" class="z-50 hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
        <div class="bg-white p-8 rounded-lg shadow-lg w-96 m-4">
            ...
            <form id="deleteForm" method="POST" action="{{ route('siswa.destroy', 0) }}">
                ...
                <div class="relative mb-4">
                    ...
                    <span toggle="#password" class="absolute right-3 top-2 cursor-pointer">
                        <i class="material-symbols-outlined text-gray-500">visibility_off</i>
                    </span>
                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#searchInput').on('keyup', function() {
                let query = $(this).val(); // Ambil input dari pencarian
                $.ajax({
                    url: "{{ route('siswa.index') }}", // Pastikan URL ini benar
                    method: "GET",
                    data: {
                        search: query
                    }, // Kirimkan query pencarian
                    success: function(response) {
                        // Update tabel dan pagination
                        $('#studentRows').html(response.tbody);
            ...
            // Handle pagination dengan AJAX
            $(document).on('click', '.pagination a', function(e) {
                e.preventDefault();
                let url = $(this).attr('href');
                let query = $('#searchInput').val(); // Ambil query pencarian
                $.ajax({
                    url: url,
                    method: 'GET',
                    data: {
                        search: query
                    }, // Pastikan query tetap dikirim saat klik pagination
                    success: function(response) {
                        $('#studentRows').html(response.tbody);
                        $('#paginationLinks').html(response.pagination);
                    },
