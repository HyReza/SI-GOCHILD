<x-app-layout>
    ...
    {{-- SweetAlert2 (jaga-jaga kalau belum di-include di layout utama) --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="time" id="arrival_time" name="arrival_time"
                        value="{{ old('arrival_time', $dailyReport->arrival_time) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-white">
                </div>
                <div>
                    ...
                    <input type="time" id="departure_time" name="departure_time"
                        value="{{ old('departure_time', $dailyReport->departure_time) }}"
                        class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 shadow-sm dark:bg-gray-700 dark:text-white">
                </div>
            </div>
    ...
    <script>
        ...
        function babyEditor({
            ...
            return {
                // state
                asiList: Array.isArray(asi) ? asi : [],
                mpList: Array.isArray(mpasi) ? mpasi : [],
                napList: Array.isArray(naps) ? naps : [],
                diaperList: Array.isArray(diapers) ? diapers : [],
                ...
                // serializer → hidden inputs (selalu ada, termasuk [])
                serializeBabyJson() {
                    const asis = (this.asiList || []).map(r => ({
                        jam: String(r.jam || '').trim(),
                        takaran: (r.takaran !== '' && r.takaran != null) ? Number(r.takaran) : null,
                        asi: !!r.asi
                    })).filter(r => r.jam || r.takaran !== null || r.asi);

                    const mp = (this.mpList || []).map(r => ({
                        jam: String(r.jam || '').trim(),
                        jumlah: String(r.jumlah || '').trim()
                    })).filter(r => r.jam || r.jumlah);

                    const ns = (this.napList || []).map(r => ({
                        tidur: String(r.tidur || '').trim(),
                        bangun: String(r.bangun || '').trim()
                    })).filter(r => r.tidur || r.bangun);

                    const dp = (this.diaperList || []).map(r => ({
                        jam: String(r.jam || '').trim(),
                        bak: !!r.bak,
                        bab: !!r.bab
                    })).filter(r => r.jam || r.bak || r.bab);

                    // set hidden inputs; jika kosong, tetap set "[]"
                    const setVal = (id, v) => {
                        const el = document.getElementById(id);
                        if (el) el.value = JSON.stringify(v);
                    };
                    setVal('asi_formula_items_json', asis);
                    setVal('mpasi_items_json', mp);
                    setVal('naps_json', ns);
                    setVal('diapers_json', dp);
                },

                // init hook
                init() {
                    // serial awal agar hidden terisi sesuai data model
                    this.serializeBabyJson();

                    // auto serialize ketika array berubah (watch sederhana)
                    this.$watch('asiList', () => this.serializeBabyJson());
                    this.$watch('mpList', () => this.serializeBabyJson());
                    this.$watch('napList', () => this.serializeBabyJson());
                    this.$watch('diaperList', () => this.serializeBabyJson());
                }
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const form = document.getElementById('dailyReportEditForm');
            const submitBtn = document.getElementById('submitFormButton');

            // cegah submit default (klik tombol)
            submitBtn?.addEventListener('click', function(e) {
                e.preventDefault();
                // ensure serialize baby json terbaru sebelum konfirmasi
                const babyBox = document.getElementById('babyEditorBox');
                if (babyBox && window.Alpine) {
                    const data = Alpine.$data(babyBox);
                    if (data && typeof data.serializeBabyJson === 'function') data.serializeBabyJson();
                }

                Swal.fire({
                    title: 'Simpan Perubahan?',
                    text: 'Pastikan data sudah benar. Perubahan akan disimpan.',
                    icon: 'question',
            ...
            // juga intercept submit via Enter
            form.addEventListener('submit', function(e) {
                // kalau submit karena confirm di atas, biarkan
