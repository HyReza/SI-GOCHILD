@php
    ...
    // Pastikan data di-decode menjadi array valid untuk AlpineJS, atau default ke array kosong item
    $asiData = $detail->asi_formula_items ?? [];
    $mpData = $detail->mpasi_items ?? [];
    $napsData = $detail->naps ?? [];
    $diapersData = $detail->diapers ?? [];

    // Jika data kosong (null/array kosong), berikan 1 baris kosong default agar tampilan tidak rusak (opsional, tapi UX lebih baik)
    if (empty($asiData)) {
        $asiData = [['jam' => '', 'takaran' => '', 'asi' => false]];
    }
    if (empty($mpData)) {
        $mpData = [['jam' => '', 'jumlah' => '']];
    }
    if (empty($napsData)) {
        $napsData = [['tidur' => '', 'bangun' => '']];
    }
    if (empty($diapersData)) {
        $diapersData = [['jam' => '', 'bak' => false, 'bab' => false]];
    }
@endphp

<div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg" x-data="{
    asiList: {{ json_encode($asiData) }},
    mpList: {{ json_encode($mpData) }},
    naps: {{ json_encode($napsData) }},
    diapers: {{ json_encode($diapersData) }}
}">

    <h2
        class="text-center text-lg font-bold text-gray-900 dark:text-gray-100 border-b border-gray-200 dark:border-gray-800 py-3">
        Edit Konsumsi & Perawatan Bayi
    </h2>
            ...
            <div class="border rounded-lg overflow-hidden border-gray-200 dark:border-gray-700">
                ...
                <div
                    ...
                    <input type="text" name="infant_breakfast_text"
                        value="{{ old('infant_breakfast_text', $detail->infant_breakfast_text) }}"
                    ...
                    <input type="text" name="infant_lunch_text"
                        value="{{ old('infant_lunch_text', $detail->infant_lunch_text) }}"
                        placeholder="Makan Siang (opsional)"
                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                    <input type="text" name="infant_dinner_text"
                        value="{{ old('infant_dinner_text', $detail->infant_dinner_text) }}"
