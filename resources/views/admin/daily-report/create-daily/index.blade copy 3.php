<x-app-layout>
            ...
            <form id="dailyReportForm" action="{{ route('daily-report.store') }}" method="POST" class="space-y-6">
                ...
                @if ($activityTransaction->service_id == 1)
                    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg">
                        ...
                        <div class="p-4" x-data="{
                            ...
                            {{-- Hidden JSON holders (diisi saat submit) --}}
                            ...
                            {{-- Serializer pada submit --}}
                            <script>
                                document.getElementById('dailyReportForm').addEventListener('submit', () => {
                                    ...
                                    const naps = (scope.naps || []).map(r => ({
                                        tidur: (r.tidur || '').trim(),
                                        bangun: (r.bangun || '').trim()
                                    })).filter(r => r.tidur || r.bangun);

                                    const diapers = (scope.diapers || []).map(r => ({
                                        jam: (r.jam || '').trim(),
                                        bak: !!r.bak,
                                        bab: !!r.bab
                                    })).filter(r => r.jam || r.bak || r.bab);

                                    document.getElementById('asi_formula_items_json').value = JSON.stringify(asis);
                                    document.getElementById('mpasi_items_json').value = JSON.stringify(mpasi);
                                    document.getElementById('naps_json').value = JSON.stringify(naps);
                                    document.getElementById('diapers_json').value = JSON.stringify(diapers);
                                });
                            </script>
                        </div>
                    </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <select id="session1_material_id" name="session1_material_id"
                                            ...
                                            @forelse ($subthemes as $subtheme)
                                                ...
                                                @endforeach
                                            @empty
                                                <option value="">Tidak ada subtema pada periode ini</option>
                                            @endforelse
                                        </select>
                ...
                <div>
                    ...
                    <div class="mt-2 space-x-6">
                        <label class="inline-flex items-center"><input type="radio" name="condition"
                                value="tenang" class="form-radio text-indigo-600"><span
                                class="ml-2">Tenang</span></label>
                        <label class="inline-flex items-center"><input type="radio" name="condition"
                                value="rewel" class="form-radio text-indigo-600"><span
                                class="ml-2">Rewel</span></label>
                        <label class="inline-flex items-center"><input type="radio" name="condition"
                                value="temper tantrum" class="form-radio text-indigo-600"><span class="ml-2">Temper
                                Tantrum</span></label>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                        *Terisi otomatis berdasarkan rentang usia & item yang belum lulus. (Tidak dapat diedit)
                    </p>
                </div>
    ...
    <script>
        ...
        // Load subthemes/materials by date (Children)
