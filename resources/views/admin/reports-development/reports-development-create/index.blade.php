    ...
    <div class="py-12 bg-gray-50 dark:bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('development-reports.store') }}" method="POST" id="reportForm">
                ...
                <input type="hidden" id="nickname" value="{{ $student->nickname }}">

                {{-- Fix Error 1364: Field doesn't have default value --}}
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm border-b dark:border-gray-700 pb-4">
                        ...
                        <div class="border-r dark:border-gray-700 pr-4 pl-2">
                            ...
                            @php
                                ...
                                $diff = $dob->diff($now); // Hitung selisih tahun & bulan
                            @endphp
                            ...
                            <div class="flex flex-col gap-1">
                                <select name="semester"
                                    ...
                                    <option value="Semester 1 ( Ganjil )"
                                        {{ $semester == 'Ganjil' ? 'selected' : '' }}>Semester 1
                                        (Ganjil)</option>
                                    <option value="Semester 2 ( Genap )" {{ $semester == 'Genap' ? 'selected' : '' }}>
                                        Semester 2
                                        (Genap)</option>
                                </select>
                ...
                <div
                    ...
                    <ul class="flex flex-wrap text-xs font-bold uppercase tracking-wider text-center" id="myTab">
                        ...
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn"
                                ...
                                Kesehatan</button></li>
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn"
                                data-target="#legal-panel" id="legal-tab-trigger"><i
                                    class="fas fa-file-signature mr-2"></i>4. Pengesahan</button>
                        </li>
                    </ul>
                </div>
                ...
                <div id="tabContent">
                    ...
                    <div class="tab-panel hidden space-y-6" id="mmdst-panel">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach ([['key' => 'personal_social', 'label' => 'Personal Sosial', 'db_key' => 'personal_social'], ['key' => 'fine_motor', 'label' => 'Motorik Halus', 'db_key' => 'fine_motor'], ['key' => 'language', 'label' => 'Bahasa', 'db_key' => 'language'], ['key' => 'gross_motor', 'label' => 'Motorik Kasar', 'db_key' => 'gross_motor']] as $asp)
                                @php
                                    // FIX: Ambil nilai result dari Controller
                                    $val = $mmdstResults[$asp['db_key']] ?? 'UNTESTABLE';
                                @endphp
                                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                    <div class="flex justify-between items-center mb-3">
                                        <h4 class="font-bold text-gray-800 text-sm uppercase">{{ $asp['label'] }}</h4>
                                        {{-- FIX: Dropdown Selected Logic --}}
                    ...
                    <div class="tab-panel hidden grid grid-cols-1 lg:grid-cols-2 gap-8" id="health-panel">
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                @foreach ($healthItems as $item)
                                    <div>
                                        <label
                                            class="text-[10px] font-bold text-gray-500 uppercase block mb-1">{{ $item }}</label>
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            <h3
                                ...
                                Rekap Absensi (Otomatis)</h3>
                            <div class="grid grid-cols-4 gap-2 mb-6">
                                @foreach ($attendanceSummary as $key => $val)
                                    @php
                                        // MAPPING MANUAL: Key Bahasa Indonesia -> Kolom Database Bahasa Inggris
                                        $dbName = 'attendance_alpha'; // Default

                                        if (strtolower($key) == 'hadir') {
                                            $dbName = 'attendance_present';
                                        } elseif (strtolower($key) == 'sakit') {
                                            $dbName = 'attendance_sick';
                                        } elseif (strtolower($key) == 'izin') {
                                            $dbName = 'attendance_permission';
                                        } elseif (strtolower($key) == 'alpha') {
                                            $dbName = 'attendance_alpha';
                                        }
                                    @endphp
                                    ...
                                    <div
                                        ...
                                        {{-- FIX: Input Hidden sekarang menggunakan nama kolom DB yang benar --}}
                    ...
                    {{-- 4. PENGESAHAN (FIX: ID Matches & TTD VISIBLE) --}}
    ...
    <script>
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // Perbaikan: ID disesuaikan dengan tombol (legal-panel)
        ...
        function resizeAllCanvases() {
            sigIds.forEach(id => {
                ...
                if (canvas && canvas.parentElement.offsetWidth > 0) {
                    ...
                    // Tidak perlu clear disini jika ingin preserve data, tapi untuk new create ok
                }
            });
        }
                            ...
                            callbacks: {
                                ...
                                label: (context) => {
                                    if (context.dataset.label === 'Pengukuran Anak') {
                                        ...
                                        return [`Tanggal: ${new Date(p.date).toLocaleDateString('id-ID')}`,
                                            `Umur: ${ageString}`, `Nilai: ${val}`, `SD: ${sd}`,
                                            `Status: ${status}`
                                        ];
                                    }
                                    return null;
        ...
        allPossibleButtons.forEach(key => {
            if (chartConfigs[key] && chartConfigs[key].standard) {
                ...
                button.type = 'button'; // PENTING: Mencegah Reload
                button.className =
        ...
        function submitFinal(e) {
            ...
            Swal.fire({
                ...
                if (r.isConfirmed) {
                    sigIds.forEach(id => {
                        if (pads[id] && !pads[id].isEmpty()) document.getElementById('input_' + id).value =
                            pads[id].toDataURL();
                    });
        ...
        console.log("Data Siswa:", studentDataPoints);
        console.log("Data Standar:", allStandards);

        if (Object.keys(allStandards).length === 0) {
            console.error("Data Standar Pertumbuhan KOSONG. Grafik tidak akan dirender.");
            document.getElementById('chart-title').textContent =
                "ERROR: Data Standar Pertumbuhan (KMS) belum tersedia di Database.";
        }
        // --- 6. CHART IMAGE GENERATOR (KEMBALI ADA) ---
