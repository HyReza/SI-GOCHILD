            ...
            <form action="{{ route('development-reports.store') }}" method="POST" id="reportForm">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm border-b dark:border-gray-700 pb-4">
                        <div class="flex items-start gap-3 border-r dark:border-gray-700 pr-4">
                            <div class="flex-shrink-0">
                                <img class="h-12 w-12 rounded-full object-cover border-2 border-indigo-100"
                        ...
                        <div class="border-r dark:border-gray-700 pr-4 pl-2">
                            ...
                            @php
                                ...
                                $diff = $dob->diff($now);
                            @endphp
                            <div class="font-bold text-gray-900 dark:text-white text-lg">{{ $diff->y }} Thn
                                {{ $diff->m }} Bln</div>
                        ...
                        <div class="pl-2">
                            ...
                            <div class="flex flex-col gap-1">
                                <select name="semester"
                                    class="w-full text-xs font-bold border-gray-300 rounded text-indigo-600 focus:ring-indigo-500">
                                ...
                                <input type="date" name="report_date" value="{{ date('Y-m-d') }}"
                                    class="w-full text-xs font-medium border-gray-300 rounded text-gray-600 focus:ring-indigo-500">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div
                    ...
                    <ul class="flex flex-wrap text-xs font-bold uppercase tracking-wider text-center" id="myTab">
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn active"
                                data-target="#kms-panel">1. Grafik Pertumbuhan</button></li>
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn"
                                data-target="#mmdst-panel">2. Perkembangan MMDST</button></li>
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn"
                                data-target="#health-panel">3. Fisik & Kesehatan</button></li>
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn" data-target="#legal-panel"
                                id="legal-tab-trigger">4. Pengesahan</button></li>
                    </ul>
                </div>
                    ...
                    <div class="tab-panel bg-white dark:bg-gray-900 shadow-md sm:rounded-lg p-6 border border-gray-100 dark:border-gray-700"
                        ...
                        <div class="mt-8 pt-4 border-t border-gray-100 dark:border-gray-700">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <button type="button" onclick="generateAI('Pertumbuhan Fisik', 'growth_desc')"
                                    class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded text-[10px] font-bold border border-indigo-200 hover:bg-indigo-100 transition">✨
                                    Generate AI</button>
                            </div>
                            <textarea name="growth_analysis_desc" id="growth_desc" oninput="autoResize(this)"
                                ...
                                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                    <div class="flex justify-between items-center mb-3">
                                        ...
                                        <select name="mmdst_{{ $asp['key'] }}_result" id="res_{{ $asp['key'] }}"
                                            class="text-[10px] font-bold rounded-full py-0.5 border-gray-300 uppercase bg-gray-50 focus:ring-indigo-500">
                                    ...
                                    <div class="relative">
                                        ...
                                        <button type="button"
                                            ...
                                            class="absolute bottom-2 right-2 text-indigo-600 bg-white border border-indigo-100 px-2 py-0.5 rounded text-[10px] font-bold shadow-sm hover:bg-indigo-50 transition">✨
                                            AI</button>
                                    </div>
                                </div>
                            @endforeach
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-4 gap-2 mb-6">
                                ...
                                @foreach ($attendanceSummary as $key => $val)
                                    <div
                                        class="bg-gray-50 p-2 rounded text-center border border-gray-200 hover:border-indigo-300 transition">
                                        <span
                                            class="block text-[9px] font-bold text-gray-400 uppercase">{{ $key }}</span>
                                        <span
                                            class="block text-2xl font-black text-indigo-600">{{ $val }}</span>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            @foreach ([['id' => 'sig_ortu', 'input' => 'parent_signature', 'name' => 'parent_name', 'label' => 'Orang Tua / Wali', 'def' => ''], ['id' => 'sig_guru', 'input' => 'teacher_signature', 'name' => 'teacher_name', 'label' => 'Wali Kelas', 'def' => $defaultTeacherName], ['id' => 'sig_konsultan', 'input' => 'consultant_signature', 'name' => 'consultant_name', 'label' => 'Konsultan', 'def' => $defaultConsultantName], ['id' => 'sig_kepsek', 'input' => 'principal_signature', 'name' => 'principal_name', 'label' => 'Kepala Sekolah', 'def' => $defaultPrincipalName]] as $sig)
                                <div class="flex flex-col items-center w-full">
                                    ...
                                    <div class="signature-pad-container group hover:border-indigo-400 transition">
                                    ...
                                    <input type="text" name="{{ $sig['name'] }}" value="{{ $sig['def'] }}"
                                        ...
                                        class="w-full mt-3 text-xs font-bold text-center border-gray-300 rounded-md focus:ring-indigo-500">
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="mt-8 flex justify-end gap-4 pb-20">
                    ...
                    <button type="submit" onclick="submitFinal(event)"
                        class="px-10 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg font-bold shadow-md hover:shadow-lg transition transform hover:-translate-y-0.5">Simpan
                        Raport</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        function initSignatures() {
            sigIds.forEach(id => {
                ...
                if (canvas) pads[id] = new SignaturePad(canvas, {
        ...
        function submitFinal(e) {
            ...
            Swal.fire({
                ...
                didOpen: async () => {
                    ...
                    await generateAllChartsImages(); // <--- Fungsi ini sekarang ada
