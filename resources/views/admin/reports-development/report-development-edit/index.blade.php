<x-app-layout>
    <x-slot:title>Edit Raport: {{ $report->student->student_name }}</x-slot:title>

    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-white leading-tight">
                {{ __('Edit Raport Perkembangan') }}
            </h2>
    ...
    <div class="py-12 bg-gray-50 dark:bg-gray-900 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('development-reports.update', $report->id) }}" method="POST" id="reportForm">
                @csrf
                ...
                <input type="hidden" id="nickname" value="{{ $report->student->nickname }}">
                <input type="hidden" name="period_start_date" value="{{ $report->period_start_date }}">
                <input type="hidden" name="period_end_date" value="{{ $report->period_end_date }}">
                <input type="hidden" name="age_in_months" value="{{ $report->age_in_months }}">
                <input type="hidden" name="academic_year" value="{{ $report->academic_year }}">
                <input type="hidden" name="mmdst_assessment_id" value="{{ $report->mmdst_assessment_id }}">

                {{-- INPUT HIDDEN GRAFIK (AKAN DIGENERATE ULANG JS) --}}
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm border-b dark:border-gray-700 pb-4">
                        <div class="flex items-start gap-3 border-r dark:border-gray-700 pr-4">
                            ...
                            <div>
                                ...
                                <div class="font-bold text-gray-900 dark:text-white">
                                    {{ $report->student->student_name }}</div>
                                <div class="text-xs text-gray-500">{{ $report->student->student_number ?? 'No ID' }}
                                </div>
                            </div>
                        </div>
                        <div class="border-r dark:border-gray-700 pr-4 pl-2">
                            ...
                            <p class="font-semibold text-gray-800 dark:text-gray-200">
                                {{ $report->student->activityTransaction->service->service_name ?? '-' }}</p>
                            <p class="text-xs text-gray-500">
                                {{ $report->student->activityTransaction->program->program_name ?? '-' }}</p>
                        </div>
                        <div class="border-r dark:border-gray-700 pr-4 pl-2">
                            ...
                            <div class="font-bold text-gray-900 dark:text-white text-lg">
                                {{ floor($report->age_in_months / 12) }} Thn {{ $report->age_in_months % 12 }} Bln
                            </div>
                        </div>
                        <div class="pl-2">
                            ...
                            <div class="flex flex-col gap-1">
                                <select name="semester"
                                    ...
                                    <option value="Ganjil" {{ $report->semester == 'Ganjil' ? 'selected' : '' }}>
                                        Semester 1 (Ganjil)</option>
                                    <option value="Genap" {{ $report->semester == 'Genap' ? 'selected' : '' }}>
                ...
                <div
                    ...
                    <ul class="flex flex-wrap text-xs font-bold uppercase tracking-wider text-center" id="myTab">
                        ...
                        <li class="flex-1"><button type="button" class="w-full p-4 tab-btn" data-target="#legal-panel"
                                id="legal-tab-trigger"><i class="fas fa-file-signature mr-2"></i>4.
                                Pengesahan</button>
                        </li>
                    </ul>
                </div>
                    ...
                    <div class="tab-panel bg-white dark:bg-gray-900 shadow-md sm:rounded-lg p-6 border border-gray-100 dark:border-gray-700"
                        ...
                        <div class="mt-8 pt-4 border-t border-gray-100 dark:border-gray-700">
                            ...
                            <textarea name="growth_analysis_desc" id="growth_analysis_desc" oninput="autoResize(this)"
                                class="w-full text-sm border-gray-300 rounded-lg focus:ring-pink-500 focus:border-pink-500">{{ $report->growth_analysis_desc }}</textarea>
                        </div>
                    </div>
                        ...
                        <div
                            ...
                            <div>
                                ...
                                <p class="text-lg font-bold text-indigo-600">
                                    {{ $report->mmdst_final_result ?? 'UNTESTABLE' }}</p>
                            </div>
                            <input type="hidden" name="mmdst_final_result"
                                value="{{ $report->mmdst_final_result }}">
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach ([['key' => 'personal_social', 'label' => 'Personal Sosial', 'db_res' => 'mmdst_personal_social_result', 'db_desc' => 'personal_social_desc'], ['key' => 'fine_motor', 'label' => 'Motorik Halus', 'db_res' => 'mmdst_fine_motor_result', 'db_desc' => 'fine_motor_desc'], ['key' => 'language', 'label' => 'Bahasa', 'db_res' => 'mmdst_language_result', 'db_desc' => 'language_desc'], ['key' => 'gross_motor', 'label' => 'Motorik Kasar', 'db_res' => 'mmdst_gross_motor_result', 'db_desc' => 'gross_motor_desc']] as $asp)
                                @php
                                    $val = $report->{$asp['db_res']} ?? 'UNTESTABLE';
                                    $desc = $report->{$asp['db_desc']} ?? '';
                                @endphp
                                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                    <div class="flex justify-between items-center mb-3">
                                        <h4 class="font-bold text-gray-800 text-sm uppercase">{{ $asp['label'] }}</h4>
                                        <select name="{{ $asp['db_res'] }}" id="res_{{ $asp['key'] }}"
                                            ...
                                            <option value="SUSPECT"
                                                {{ $val == 'SUSPECT' || $val == 'QUESTIONABLE' || $val == 'CAUTION' ? 'selected' : '' }}>
                                    ...
                                    <div class="relative">
                                        <textarea name="{{ $asp['db_desc'] }}" id="desc_{{ $asp['key'] }}" oninput="autoResize(this)"
                                            class="w-full rounded-md border-gray-300 shadow-sm text-sm focus:ring-pink-500 focus:border-pink-500">{{ $desc }}</textarea>
                                        <button type="button"
                    ...
                    <div class="tab-panel hidden grid grid-cols-1 lg:grid-cols-2 gap-8" id="health-panel">
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                ...
                                @endphp
                                @foreach ($healthMap as $label => $col)
                                    <div>
                                        <label
                                            class="text-[10px] font-bold text-gray-500 uppercase block mb-1">{{ $label }}</label>
                                        <select name="health[{{ $label }}]"
                                            class="w-full text-xs font-semibold rounded border-gray-300">
                                            @php $val = $report->healthDetail->$col ?? 'Baik'; @endphp
                                            ...
                                            <option value="Dalam Perawatan"
                                                {{ $val == 'Dalam Perawatan' ? 'selected' : '' }}>Dalam Perawatan
                                            </option>
                                        </select>
                                    </div>
                                @endforeach
                        ...
                        <div class="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                            ...
                            <div class="grid grid-cols-4 gap-2 mb-6">
                                @foreach ([
        'Hadir' => ['col' => 'attendance_present', 'color' => 'green'],
        'Sakit' => ['col' => 'attendance_sick', 'color' => 'blue'],
        'Izin' => ['col' => 'attendance_permission', 'color' => 'yellow'],
        'Alpha' => ['col' => 'attendance_alpha', 'color' => 'red'],
    ] as $label => $data)
                                    <div
                                        class="bg-gray-50 p-2 rounded text-center border border-gray-200 hover:border-{{ $data['color'] }}-300 transition">
                                        <span
                                            class="block text-[9px] font-bold text-gray-400 uppercase">{{ $label }}</span>
                                        <span
                                            class="block text-2xl font-black text-{{ $data['color'] }}-600">{{ $report->{$data['col']} }}</span>
                                        <input type="hidden" name="{{ $data['col'] }}"
                                            value="{{ $report->{$data['col']} }}">
                                    </div>
                                @endforeach
                            </div>
                            <label class="text-[10px] font-bold text-gray-400 block mb-1 uppercase">Catatan
                                Guru</label>
                            <textarea name="teacher_notes" oninput="autoResize(this)" class="w-full text-sm border-gray-300 rounded mb-3">{{ $report->teacher_notes }}</textarea>
                            <label class="text-[10px] font-bold text-gray-400 block mb-1 uppercase">Rekomendasi</label>
                            <textarea name="teacher_recommendations" oninput="autoResize(this)" class="w-full text-sm border-gray-300 rounded">{{ $report->teacher_recommendations }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="tab-panel hidden bg-white rounded-xl p-8 shadow-sm border border-gray-200"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            @foreach ([['id' => 'sig_ortu', 'input' => 'parent_signature', 'name' => 'parent_name', 'label' => 'Orang Tua / Wali', 'db_name' => $report->parent_name, 'db_sig' => $report->parent_signature], ['id' => 'sig_guru', 'input' => 'teacher_signature', 'name' => 'teacher_name', 'label' => 'Wali Kelas', 'db_name' => $report->teacher_name, 'db_sig' => $report->teacher_signature], ['id' => 'sig_konsultan', 'input' => 'consultant_signature', 'name' => 'consultant_name', 'label' => 'Konsultan', 'db_name' => $report->consultant_name, 'db_sig' => $report->consultant_signature], ['id' => 'sig_kepsek', 'input' => 'principal_signature', 'name' => 'principal_name', 'label' => 'Kepala Sekolah', 'db_name' => $report->principal_name, 'db_sig' => $report->principal_signature]] as $sig)
                                <div class="flex flex-col items-center w-full">
                                    ...
                                    <div class="signature-container group hover:border-pink-400 transition"
                                        style="{{ $sig['db_sig'] ? 'background-image: url(' . asset('storage/' . $sig['db_sig']) . '); background-size: contain; background-repeat: no-repeat; background-position: center;' : '' }}">
                                    ...
                                    <input type="hidden" name="{{ $sig['input'] }}"
                                        id="input_{{ $sig['id'] }}">
                                    <input type="text" name="{{ $sig['name'] }}" value="{{ $sig['db_name'] }}"
                ...
                <div class="mt-8 flex justify-end gap-4 pb-20">
                    ...
                    <button type="submit" onclick="submitFinal(event)"
                        class="px-10 py-3 bg-gradient-to-r from-pink-600 to-purple-600 text-white rounded-lg font-bold shadow-md hover:shadow-lg transition transform hover:-translate-y-0.5">Update
                        Raport</button>
                </div>
            </form>
        </div>
    ...
    <script>
        ...
        document.querySelectorAll('.tab-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                // FIX TANDA TANGAN: Resize canvas saat tab dibuka
        ...
        window.clearPad = function(id) {
            if (pads[id]) {
                ...
                // 2. Hilangkan Background Gambar Lama (Visual)
                document.getElementById(id).parentElement.style.backgroundImage = 'none';

                // 3. ISI INPUT HIDDEN DENGAN "DELETE" (Sinyal ke Controller)
                document.getElementById('input_' + id).value = 'DELETE';
            }
        }
        ...
        // 3. CHART LOGIC (SAMA PERSIS DENGAN CREATE)
        const studentDataPoints = @json($chartData ?? []);
        const allStandards = @json($allStandardCurves ?? []);
        const studentBirthDate = '{{ $report->student->birth_date }}';
        const isMale = {{ $report->student->gender == 1 || $report->student->gender == 'male' ? 'true' : 'false' }};
                ...
                data: {
                    datasets: [{
                        ...
                        },
                        {
                            ...
                            data: createLineBreaks(config.dataSource.map(d => ({
                                ...d,
                                x: d[config.xKey],
                                y: d[config.yKey]
                            })), 'x', 'y'),
                            ...
                            tension: 0.1,
                            borderWidth: 3,
                            // Parsing penting agar tooltip bisa baca data asli
                            parsing: {
                                xAxisKey: 'x',
                                yAxisKey: 'y'
                            }
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                title: () => '',
                                label: (context) => {
                                    // TOOLTIP LOGIC LENGKAP
                                    if (context.dataset.label === 'Pengukuran Anak') {
                                        const p = context.raw;
                                        if (!p || isNaN(p.y)) return null;

                                        const age = getFullAge(studentBirthDate, p.date);
                                        const ageString = age ? `${age.years} thn, ${age.months} bln` : '-';
                                        const val = p.y;

                                        const chartKey = config.chartKey; // Kunci grafik (BB/U, dll)
                                        // Ambil Data SD & Status Gizi
                                        const sd = (p.sd_category && p.sd_category[chartKey]) ? p.sd_category[
                                            chartKey] : 'N/A';
                                        const status = (p.status_gizi && p.status_gizi[chartKey]) ? p
                                            .status_gizi[chartKey] : 'N/A';
        ...
        async function generateAI(category, targetId, resId = null) {
            const student = document.getElementById('nickname').value;
        ...
        // 6. GENERATE CHART IMAGES (SAMA PERSIS DENGAN CREATE)
        async function generateAllChartsImages() {
            ...
            for (let target of targets) {
                ...
                const xLabel = config.standard.x_axis_key === 'age_months' ? 'Umur (bulan)' : (isBaby ? 'Panjang (cm)' :
                    'Tinggi (cm)');
                const mapStd = (data) => data ? data : [];
                const points = createLineBreaks(config.dataSource.map(d => ({
                    ...d,
                    x: d[config.xKey],
                    y: d[config.yKey]
                })), 'x', 'y');
        ...
        function submitFinal(e) {
            ...
            Swal.fire({
                title: 'Update Data?',
                text: 'Data lama akan ditimpa dengan data baru ini.',
                icon: 'warning',
                ...
                if (r.isConfirmed) {
                    Swal.fire({
                        ...
                        didOpen: async () => {
                            ...
                            // 1. PROSES TANDA TANGAN
                            sigIds.forEach(id => {
                                const hiddenInput = document.getElementById('input_' + id);

                                // Jika user menggambar tanda tangan baru
                                if (pads[id] && !pads[id].isEmpty()) {
                                    hiddenInput.value = pads[id].toDataURL();
                                }
                                // Jika pad kosong, biarkan nilai hiddenInput apa adanya.
                                // (Bisa jadi '' atau 'DELETE' jika tombol hapus ditekan tadi)
                            });

                            // 2. Generate Grafik Baru (Wajib update grafik)
                            await new Promise(r => setTimeout(r, 100));
                            await generateAllChartsImages();
