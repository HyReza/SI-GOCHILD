<x-app-layout>
    ...
    <script>
        ...
        // Cek jika Sub-Theme sudah digunakan dalam materi
        let isUsedInMaterial = {{ $subTheme->material()->count() > 0 ? 'true' : 'false' }};

        if (isUsedInMaterial) {
            document.getElementById('theme_id').disabled = true; // Disable theme dropdown if used in material
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Sub Tema ini sudah digunakan dalam Materi dan tidak dapat diubah.',
