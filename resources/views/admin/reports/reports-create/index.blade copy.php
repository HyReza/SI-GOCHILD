    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @php
                ...
                // Peta relasi [subThemeId => themeId]
                $subThemeToThemeMap = [];
                // Peta relasi [materialId => themeId]
                $materialToThemeMap = [];
            ...
            <div x-data="{
                ...
                materialToThemeMap: {{ json_encode($materialToThemeMap) }}, // Peta baru ditambahkan
            
                // Inisialisasi data skor awal dari server
                init() {
                    let initialScores = {};
                ...
                // ✅ FUNGSI YANG DIPERBAIKI TOTAL: Logika ditulis ulang dengan JavaScript murni
                generateThemeNote(themeId) {
                    let themeScores = [];
            
                    // 1. Ambil skor Tema itu sendiri
                    if (this.scores['theme_' + themeId]) themeScores.push(this.scores['theme_' + themeId]);
            
                    // 2. Iterasi semua skor yang ada untuk mencari yang relevan
                ...
                // (Fungsi generateOverallSummary dan handleSubmit tetap sama)
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 md:p-8">
                    ...
                    <form @submit.prevent="handleSubmit">
                        ...
                        <div class="min-h-[450px]">
                            {{-- Ganti SELURUH BLOK @foreach untuk tema ini --}}
                            @foreach ($data['themes'] as $theme)
                                <div x-show="step === {{ $loop->iteration }}" x-cloak
                                                    ...
                                                    @foreach (['BB', 'MB', 'BSH', 'BSB'] as $score)
                                                        <td class="px-2 py-3 text-center">
                                                            <input type="radio"
                                                                ...
                                                                name="scores[theme][{{ $theme->id }}][score]"
                                                                value="{{ $score }}">
                                                        </td>
                                                    @endforeach
                                                </tr>

                                                {{-- Looping Sub Tema dan Materi --}}
                                                @foreach ($theme->subTheme as $subTheme)
                                                    <tr class="bg-gray-50">
                                                        <td
                                                        ...
                                                        @foreach (['BB', 'MB', 'BSH', 'BSB'] as $score)
                                                            <td class="px-2 py-2.5 text-center">
                                                                <input type="radio"
                                                                    ...
                                                                    @change="generateThemeNote({{ $theme->id }}); generateOverallSummary()"
                                                                    name="scores[subtheme][{{ $subTheme->id }}][score]"
                                                                    value="{{ $score }}">
                                                            </td>
                                                        @endforeach
                                                    </tr>
                                                            ...
                                                            @foreach (['BB', 'MB', 'BSH', 'BSB'] as $score)
                                                                <td class="px-2 py-2.5 text-center">
                                                                    ...
                                                                    <input type="radio"
                                                                        ...
                                                                        @change="generateThemeNote({{ $theme->id }}); generateOverallSummary()"
                                                                        name="scores[material][{{ $material->id }}][score]"
                                                                        value="{{ $score }}">
                                                                    <input type="hidden"
                                                                        name="scores[material][{{ $material->id }}][sub_theme_id]"
                                                                        value="{{ $subTheme->id }}">
                                                                </td>
                                                            @endforeach
                                                        </tr>
                                    ...
                                    <div>
                                        <label for="note_{{ $loop->index }}"
                                            ...
                                            (Otomatis)</label>
                                        {{-- DIUBAH: Menggunakan format array numerik --}}
                        ...
                        <div class="flex justify-between mt-8 pt-6 border-t">
                            <button type="button" x-show="step > 1" @click="step--"
                                class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 disabled:opacity-25 transition">
                                Kembali
                            </button>
                            ...
                            <button type="submit" x-show="step === totalSteps"
                                class="inline-flex items-center px-6 py-3 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:outline-none focus:border-green-900 focus:ring ring-green-300 disabled:opacity-25 transition">
                                Simpan Rapor
                            </button>
                        </div>
                    </form>
