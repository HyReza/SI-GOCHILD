    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <script>
                    document.addEventListener('DOMContentLoaded', function() {
                        Swal.fire({
                            ...
                            text: 'Harap periksa kembali isian form yang berwarna merah.',
            ...
            <form action="{{ route('reports.store') }}" method="POST" enctype="multipart/form-data" id="reportForm">
                ...
                {{-- Input Hidden untuk Nama yang digunakan di JS (Draft & AI) --}}
                <input type="hidden" id="student_calling_name" value="{{ $callingName }}">

                <input type="hidden" name="start_date" value="{{ $startDate }}">
                <input type="hidden" name="end_date" value="{{ $endDate }}">

                {{-- Hidden untuk menyimpan JSON absensi ke database --}}
                        ...
                        <div class="col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            <div>
                                ...
                                <input type="text" name="report_title"
                                    value="{{ old('report_title', 'Laporan Capaian Pembelajaran') }}"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm transition duration-150 @error('report_title') input-error @enderror"
                                    required>
                                @error('report_title')
                            ...
                            <div>
                                ...
                                <select name="semester"
                                    ...
                                    <option value="Semester 1 (Ganjil)"
                                        {{ old('semester') == 'Semester 1 (Ganjil)' ? 'selected' : '' }}>Semester 1
                                        (Ganjil)</option>
                                    <option value="Semester 2 (Genap)"
                                        {{ old('semester') == 'Semester 2 (Genap)' ? 'selected' : '' }}>Semester 2
                                        (Genap)</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="class_name" id="class_name"
                                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('class_name') input-error @enderror"
                                    required>
                                    <option value="" disabled {{ old('class_name') ? '' : 'selected' }}>-- Pilih
                                        Kelas --</option>
                                    <option value="Early Childhood Day Care A"
                                        {{ old('class_name') == 'Early Childhood Day Care A' ? 'selected' : '' }}>Early
                                        Childhood Day Care A</option>
                                    <option value="Early Childhood Day Care B"
                                        {{ old('class_name') == 'Early Childhood Day Care B' ? 'selected' : '' }}>Early
                                        Childhood Day Care B</option>
                                </select>
                                @error('class_name')
                                    <p class="text-red-500 text-xs mt-1">Kelas wajib dipilih</p>
                                @enderror
                            </div>
                            <div>
                ...
                <div id="myTabContent">
                    ...
                    <div class="rounded-lg bg-white shadow-sm border border-gray-100 p-6" id="profile"
                            ...
                            <div class="bg-gray-50 px-5 py-4 border-b border-gray-200 flex justify-between items-center cursor-pointer hover:bg-gray-100 transition"
                                ...
                                <div>
                                    <h3 class="font-bold text-gray-800 text-lg">1. Detail Penilaian Kurikulum
                                        (Checklist)</h3>
                                    <p class="text-xs text-gray-500">Klik untuk melihat detail skor per materi</p>
                                </div>
                                                ...
                                                @foreach ($theme->subThemes as $subTheme)
                                                    <div class="ml-4 mb-4 pl-4 border-l-2 border-indigo-100">
                                                        ...
                                                        @foreach ($subTheme->materials as $material)
                                                            @php
                                                                // Ambil data nilai dari controller
                                                                $dataScore = $calculatedScores[$material->id] ?? [
                                                                    'score' => null,
                                                                    'dates' => [],
                                                                    'info' => 'Belum ada data harian',
                                                                ];
                                                                $currentScore = $dataScore['score'];
                                                                $historyDates = $dataScore['dates'] ?? [];
                                                                $infoHistory = $dataScore['info'];
                                                                $hasHistory = !empty($historyDates);
                                                            @endphp
                                                                ...
                                                                <div
                                                                    ...
                                                                    <div class="w-full md:w-2/3">
                                                                        <div class="flex items-start">
                                                                            <i
                                                                                class="fas fa-check-circle text-gray-300 mt-1 mr-2 text-xs"></i>
                                                                            <div>
                                                                                <span
                                                                                ...
                                                                                <div class="mt-1">
                                                                                    @if (is_null($currentScore))
                                                                                        <span
                                                                                            class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-yellow-100 text-yellow-800 border border-yellow-200">
                                                                                            <i
                                                                                    ...
                                                                                    @else
                                                                                        <span
                                                                                            class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-800 border border-green-200">
                                                                                            <i
                                                                        ...
                                                                        @if ($hasHistory)
                                                                            <div
                                                                                class="mt-2 text-[10px] text-gray-500 bg-gray-50 p-1.5 rounded border border-gray-100 inline-block">
                                                                                <strong class="text-gray-600"><i
                                                                                        class="fas fa-history mr-1"></i>
                                                                                    Riwayat Penilaian:</strong><br>
                                                                                <span class="leading-relaxed">
                                                                                    {{ implode(', ', array_unique($historyDates)) }}
                                                                                </span>
                                                                            </div>
                                                                        @endif
                                                                    </div>
                        ...
                        <div class="space-y-6 mb-8">
                            <div class="border-b-2 border-indigo-500 pb-2">
                                ...
                                <p class="text-xs text-gray-500 mt-1">Gunakan tombol generator untuk mengubah checklist
                                    di atas menjadi narasi paragraf yang mengalir.</p>
                            </div>
                                ...
                                <div
                                    ...
                                    <div class="flex justify-between items-center mb-3">
                                        ...
                                        <button type="button"
                                            ...
                                            <i class="fas fa-robot mr-1.5"></i> Buat Narasi Otomatis
                                        </button>
                                    </div>
                                    <textarea name="theme_notes[{{ $theme->id }}]" id="theme_note_{{ $theme->id }}" rows="4"
                                        ...
                                        placeholder="Klik tombol 'Buat Narasi Otomatis' untuk menghasilkan teks berdasarkan nilai checklist..."></textarea>
                                </div>
                            @endforeach
                        </div>
                                            ...
                                            <div
                                                ...
                                                <div id="upload_box_{{ $sec['photo'] }}">
                                                    <div class="mt-2 cursor-pointer">
                                                        ...
                                                        <span class="block text-xs text-gray-500">Klik untuk
                                                            upload</span>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <textarea name="teacher_notes" rows="4" oninput="autoResize(this)"
                                        ...
                                        placeholder="Contoh: Ananda sangat antusias dalam kegiatan seni..."></textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="recommendations" rows="4" oninput="autoResize(this)"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 bg-white"
                                        placeholder="Contoh: Mohon Ayah/Bunda melatih kemandirian ananda di rumah..."></textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="hidden p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="health" role="tabpanel">
                        ...
                        {{-- === BAGIAN BARU: INFORMASI ABSENSI OTOMATIS === --}}
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mt-6">
                            <h3 class="text-lg font-bold text-gray-800 mb-4 flex items-center border-b pb-3">
                                ...
                                Presensi (Otomatis dari Data Harian)
                            </h3>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                ...
                                <div class="bg-blue-50 p-4 rounded-xl text-center border border-blue-100">
                                    <div class="text-2xl font-bold text-blue-700">
                                        {{ $attendanceSummary['Hadir'] ?? 0 }}</div>
                                    <div class="text-[10px] text-blue-500 uppercase font-bold tracking-wider">Hadir
                                    </div>
                                </div>
                                ...
                                <div class="bg-yellow-50 p-4 rounded-xl text-center border border-yellow-100">
                                    <div class="text-2xl font-bold text-yellow-700">
                                        {{ $attendanceSummary['Sakit'] ?? 0 }}</div>
                                    <div class="text-[10px] text-yellow-500 uppercase font-bold tracking-wider">Sakit
                                    </div>
                                </div>
                                ...
                                <div class="bg-orange-50 p-4 rounded-xl text-center border border-orange-100">
                                    <div class="text-2xl font-bold text-orange-700">
                                        {{ $attendanceSummary['Izin'] ?? 0 }}</div>
                                    <div class="text-[10px] text-orange-500 uppercase font-bold tracking-wider">Izin
                                    </div>
                                </div>
                                ...
                                <div class="bg-red-50 p-4 rounded-xl text-center border border-red-100">
                                    <div class="text-2xl font-bold text-red-700">
                                        {{ $attendanceSummary['Alpha'] ?? 0 }}</div>
                                    <div class="text-[10px] text-red-500 uppercase font-bold tracking-wider">Tanpa
                                        Keterangan</div>
                                </div>
                            </div>

                            <p class="text-xs text-gray-400 mt-3 italic">
                                * Data dihitung otomatis berdasarkan rentang tanggal:
                                {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} s.d
                                {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }}
                            </p>
                        </div>
                        {{-- === AKHIR BAGIAN ABSENSI === --}}
    ...
    <script>
        ...
        function generateThemeDraft(themeId, targetId) {
            ...
            scores.forEach(radio => {
                ...
                if (groups[score]) {
                    groups[score].push(materialName.toLowerCase()); // lowercase agar menyatu di kalimat
                }
            });
            ...
            // Buat kalimat untuk setiap grup yang ada isinya
            // Prioritas urutan: BSB -> BSH -> MB -> BB
            if (groups['BSB'].length > 0) {
                sentences.push(`Ananda ${callingName} ${predicates['BSB']} ${joinWords(groups['BSB'])}.`);
            }
            if (groups['BSH'].length > 0) {
                const prefix = sentences.length > 0 ? `Selain itu, Ananda ${callingName} ` : `Ananda ${callingName} `;
                sentences.push(`${prefix}${predicates['BSH']} ${joinWords(groups['BSH'])}.`);
            }
            if (groups['MB'].length > 0) {
                const prefix = sentences.length > 0 ? `Ananda ${callingName} juga ` : `Ananda ${callingName} `;
                sentences.push(`${prefix}${predicates['MB']} ${joinWords(groups['MB'])}.`);
            }
            if (groups['BB'].length > 0) {
                sentences.push(`Namun, Ananda ${callingName} ${predicates['BB']} ${joinWords(groups['BB'])}.`);
            }
            ...
            Swal.fire({
                ...
                position: 'top-end',
                icon: 'success',
                title: 'Narasi berhasil dibuat ulang',
        ...
        function generateNarrativeAI(category, targetId) {
            const studentName = document.getElementById('student_calling_name').value; // GUNAKAN NAMA PANGGILAN
            const textArea = document.getElementById(targetId);
            ...
            fetch("{{ route('generate.narrative') }}", {
                    ...
                    body: JSON.stringify({
                        student_name: studentName, // KIRIM NAMA PANGGILAN
                        category: category,
        ...
        function submitForm(e) {
            ...
            // Validasi Manual: Cek apakah kelas sudah dipilih
