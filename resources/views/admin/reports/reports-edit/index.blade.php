                ...
                <div id="myTabContent">
                    ...
                    <div class="hidden p-4 rounded-lg bg-gray-50 dark:bg-gray-800" id="health" role="tabpanel">
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div>
                                <div class="bg-white shadow-lg sm:rounded-xl p-6 border border-gray-200 mb-6">
                                    ...
                                    {{-- @php
                                        $lk_val = null;
                                        // Simpan teks asli ke variabel baru untuk dibersihkan
                                        $clean_development_text = $report->development_info_text;

                                        if ($report->development_info_text) {
                                            // 1. Ambil Angka untuk input number (Logic lama Anda)
                                            // Regex disesuaikan agar lebih fleksibel menangkap format
                                            preg_match(
                                                '/\(Lingkar Kepala:\s*([\d\.,]+)\s*cm\)/i',
                                                $report->development_info_text,
                                                $matches,
                                            );
                                            $lk_val = $matches[1] ?? null;

                                            // 2. Hapus Teks "(Lingkar Kepala: ... cm)" agar tidak muncul di textarea
                                            // Menggunakan regex untuk menghapus teks tersebut beserta spasi di depannya
                                            $clean_development_text = preg_replace(
                                                '/\s*\(Lingkar Kepala:\s*[\d\.,]+\s*cm\)/i',
                                                '',
                                                $report->development_info_text,
                                            );

                                            // Trim untuk menghapus spasi berlebih di awal/akhir
                                            $clean_development_text = trim($clean_development_text);
                                        }
                                    @endphp --}}
                                    ...
                                    <div class="grid grid-cols-3 gap-4 mb-6">
                                        ...
                                        <div>
                                            ...
                                            <input type="number" step="0.1" name="lingkar_kepala"
                                                id="lingkar_kepala"
                                                value="{{ old('lingkar_kepala', $report->head_circumference + 0) }}"
                                                class="focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md p-2 bg-gray-50 font-semibold">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mt-5">
                                        ...
                                        <div id="preview_container_kms"
                                            ...
                                            <img src="" id="img_prev_development_info_photo"
                                                alt="Preview KMS" class="w-full h-auto rounded shadow-sm">
                                            <button type="button"
                                    ...
                                    <div class="space-y-4">
                                        @foreach ($healthItems as $index => $item)
                                            <div
                                                ...
                                                <span
                                                    class="text-sm font-medium text-gray-700">{{ $item }}</span>
                                                <select name="health[{{ $item }}]"
                                                    ...
                                                    <option value="Cukup" {{ $val == 'Cukup' ? 'selected' : '' }}>
                                                        Cukup</option>
                                                    <option value="Perlu Perhatian"
                                                        {{ $val == 'Perlu Perhatian' ? 'selected' : '' }}>Perlu
                                                        Perhatian</option>
                                ...
                                <div class="bg-white shadow-lg sm:rounded-xl p-6 border border-gray-200">
                                    ...
                                    <div class="grid grid-cols-2 gap-3 mb-4">
                                        <div class="bg-blue-50 p-3 rounded-xl text-center border border-blue-100">
                                            <div class="text-2xl font-bold text-blue-700">{{ $hadir }}</div>
                                        ...
                                        <div class="bg-yellow-50 p-3 rounded-xl text-center border border-yellow-100">
                                            <div class="text-2xl font-bold text-yellow-700">{{ $sakit }}</div>
                                            <div class="text-[10px] text-yellow-500 uppercase font-bold">Sakit</div>
                                        </div>
                                        <div class="bg-purple-50 p-3 rounded-xl text-center border border-purple-100">
                                            <div class="text-2xl font-bold text-purple-700">{{ $izin }}</div>
                                            <div class="text-[10px] text-purple-500 uppercase font-bold">Izin</div>
                                        </div>
                                        <div class="bg-red-50 p-3 rounded-xl text-center border border-red-100">
                                            <div class="text-2xl font-bold text-red-700">{{ $alpha }}</div>
                                            <div class="text-[10px] text-red-500 uppercase font-bold">Tanpa Keterangan
                                            </div>
                                        </div>
                                    </div>

                                    <div class="bg-gray-50 p-4 rounded-lg text-xs text-gray-600 italic">
                                        * Data presensi di atas diambil otomatis dari rekapitulasi saat raport ini
                                        dibuat.
                                        Jika ingin mengubah data presensi, silakan lakukan edit data kehadiran terlebih
                                        dahulu, lalu buat ulang raport.
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.clear-sig-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    // Set value hidden input to DELETE
        ...
        function submitForm(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    canvasIds.forEach(id => {
                        const inputHidden = document.getElementById('input_' + id);

                        // Jika canvas ada isinya (baru)
                        if (!pads[id].isEmpty()) {
                            inputHidden.value = pads[id].toDataURL('image/png');
                        }
                        // Jika canvas kosong DAN belum ada flag DELETE, kosongkan value agar tidak dikirim (controller pakai data lama)
                        else if (inputHidden.value !== "DELETE") {
                            inputHidden.value = "";
                        }
                    });
