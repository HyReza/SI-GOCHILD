<x-app-layout>
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Formulir Rapor') }}
        </h2>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            {{-- Blok PHP untuk menyiapkan data --}}
            @php
                $totalSteps = $themes->count() + 1;
                ...
                // Memetakan data yang sudah tersimpan agar mudah diakses
            ...
            <div x-data="{
                ...
                // Inisialisasi: Mengisi form dengan data dari rapor yang sudah ada
                init() {
                    let initialScores = {};
                    @foreach ($report->details as $detail)
                        @if ($detail->theme_id)
                            initialScores['theme_{{ $detail->theme_id }}'] = '{{ $detail->score }}';
                        @elseif($detail->sub_theme_id && !$detail->material_id)
                            initialScores['subtheme_{{ $detail->sub_theme_id }}'] = '{{ $detail->score }}';
                        @elseif($detail->material_id)
                            initialScores['material_{{ $detail->material_id }}'] = '{{ $detail->score }}';
                        @endif @endforeach
                    this.scores = initialScores;
            
                    let initialThemeNotes = {};
                    @foreach ($report->themeNotes as $note)
                        initialThemeNotes['theme_{{ $note->theme_id }}'] = `{!! addslashes(e($note->note)) !!}`; @endforeach
                    this.themeNotes = initialThemeNotes;
            
                    this.overallSummary = `{!! addslashes(e($report->overall_summary)) !!}`;
                },
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 mb-8 border-l-4 border-yellow-500">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">MENGEDIT RAPOR - IDENTITAS PESERTA DIDIK</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4 text-sm">
                        <span><strong class="font-medium text-gray-500 block">Nama Lengkap</strong> <span
                                class="text-gray-800">{{ $report->activityTransaction->student->student_name }}</span></span>
                        <span><strong class="font-medium text-gray-500 block">Nomor Induk</strong> <span
                                class="text-gray-800">{{ $report->activityTransaction->student->student_number }}</span></span>
                        <span><strong class="font-medium text-gray-500 block">Periode</strong> <span
                                class="text-gray-800">{{ \Carbon\Carbon::parse($report->start_date)->isoFormat('D MMM Y') }}
                                - {{ \Carbon\Carbon::parse($report->end_date)->isoFormat('D MMM Y') }}</span></span>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 md:p-8">
                    ...
                    <form @submit.prevent="handleSubmit" method="POST"
                        ...
                        <div class="mb-6">
                            <label for="report_title" class="block font-medium text-sm text-gray-700">Judul
                                Rapor</label>
                                ...
                                <div x-show="step === {{ $loop->iteration }}" x-cloak
                                    ...
                                    <div>
                                        <label for="note_{{ $loop->index }}"
                                            class="block font-medium text-sm text-gray-700">Catatan Narasi</label>
                            ...
                            <div x-show="step === totalSteps" x-cloak class="animate-fade-in space-y-6">
                                <div class="bg-gray-50 border border-gray-200 rounded-lg p-6">
                                    ...
                                    <textarea id="overall_summary_display" x-model="overallSummary" rows="5"
                                        class="mt-2 block w-full border-gray-300 rounded-md shadow-sm bg-white text-sm" readonly></textarea>
                                </div>
                                <div class="bg-gray-50 border border-gray-200 rounded-lg p-6">
                                    ...
                                    <textarea id="recommendations" name="recommendations" rows="5"
                                        class="block w-full border-gray-300 rounded-md shadow-sm">{{ old('recommendations', $report->recommendations) }}</textarea>
                                </div>
                                <div class="bg-gray-50 border border-gray-200 rounded-lg p-6">
                                        ...
                                        <div class="space-y-4">
                                            ...
                                            @foreach ($healthItems as $index => $item)
                                                <div class="flex items-center justify-between">
                                                    ...
                                                    @php $selectedValue = old('health_details.'.$index.'.item_value', $healthDetailsMap->get($item)?->item_value); @endphp
                                                    <select name="health_details[{{ $index }}][item_value]"
                                                        class="border-gray-300 rounded-md shadow-sm text-sm py-1"
                                                        style="width: 120px;">
                                                        <option value="Baik"
                                                            @if ($selectedValue == 'Baik') selected @endif>Baik
                                                        </option>
                                                        <option value="Cukup"
                                                            @if ($selectedValue == 'Cukup') selected @endif>Cukup
                                                        </option>
                                                        <option value="Kurang"
                                                            @if ($selectedValue == 'Kurang') selected @endif>Kurang
                                                        </option>
                                                    </select>
                                                </div>
                                            @endforeach
                                        </div>
                                        {{-- Data Presensi tidak bisa diedit, hanya ditampilkan dari data asli rapor --}}
                                        <div class="space-y-4">
                                            <h4 class="font-medium text-gray-800 border-b pb-2">2. Keterangan Presensi
                                                (Data Asli)</h4>
                                            @php $attendance = json_decode($report->attendance_summary, true) ?? ['sick' => 0, 'excused' => 0, 'absent' => 0]; @endphp
                                            <div class="flex justify-between text-sm"><span
                                                    class="text-gray-700">Sakit</span><span
                                                    class="font-semibold text-gray-800">{{ $attendance['sick'] ?? 0 }}
                                                    hari</span></div>
                                            <div class="flex justify-between text-sm"><span
                                                    class="text-gray-700">Izin</span><span
                                                    class="font-semibold text-gray-800">{{ $attendance['excused'] ?? 0 }}
                                                    hari</span></div>
                                            <div class="flex justify-between text-sm"><span
                                                    class="text-gray-700">Tanpa Keterangan</span><span
                                                    class="font-semibold text-gray-800">{{ $attendance['absent'] ?? 0 }}
                                                    hari</span></div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="flex justify-between mt-8 pt-6 border-t">
                            <button type="button" x-show="step > 1" @click="step--"
                                class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">Kembali</button>
                            <div x-show="step === 1" class="w-0"></div>
                            <button type="button" x-show="step < totalSteps" @click="step++"
                                class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700">Selanjutnya</button>
                            {{-- Tombol Update Rapor --}}
                            <button type="submit" x-show="step === totalSteps"
                                class="inline-flex items-center px-6 py-3 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">Update
                                Rapor</button>
                        </div>
                    </form>
                </div>
