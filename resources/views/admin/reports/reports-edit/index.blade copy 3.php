<x-app-layout>
    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('reports.update', $report->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <input type="hidden" name="student_id" id="student_id" value="{{ $report->student_id }}">
                <input type="hidden" id="student_name" value="{{ $report->student->student_name }}">
                ...
                @php
                    // --- INISIALISASI DEFAULT (MENGHINDARI UNDEFINED VARIABLE ERROR) ---
                    $defaultTeacherName = $defaultTeacherName ?? 'Wali Kelas';
                    $defaultPrincipalName = $defaultPrincipalName ?? 'Kepala Sekolah';
                    $defaultConsultantName = $defaultConsultantName ?? 'Konsultan';

                    // Ekstrak Lingkar Kepala dan hitung Tahun Ajaran untuk tampilan
                    $lk_value = null;
                    if ($report->development_info_text) {
                        preg_match('/\((Lingkar Kepala: )([\d\.]+) cm\)/', $report->development_info_text, $matches);
                        ...
                        <div class="col-span-1 flex items-center space-x-4 border-r border-gray-100 pr-4">
                            <div class="flex-shrink-0">
                                <img class="h-16 w-16 rounded-full object-cover border-2 border-indigo-100"
                                    src="https://ui-avatars.com/api/?name={{ $report->student->student_name }}&background=random"
                                    alt="Foto Siswa">
                            </div>
                            <div>
                                <h3 class="text-lg font-bold text-gray-900">{{ $report->student->student_name }}</h3>
                                <p class="text-sm text-gray-500">{{ $report->student->student_number }}</p>
                                <span
                ...
                <div id="myTabContent">
                    ...
                    <div class="rounded-lg bg-white shadow-sm border border-gray-100 p-6" id="profile"
                                                ...
                                                @foreach ($theme->subThemes as $subTheme)
                                                    <div class="ml-2 mb-3 border-l-2 border-indigo-100 pl-3 mt-2">
                                                        ...
                                                        @foreach ($subTheme->materials as $material)
                                                            @php
                                                                $score = $savedScores[$material->id] ?? null;
                                                                $themeId = $theme->id;
                                                            @endphp
                                                            <div
                                                                    ...
                                                                    @foreach (['BB', 'MB', 'BSH', 'BSB'] as $opt)
                                                                        <label class="cursor-pointer relative">
                                                                            <input type="radio"
                                                                                ...
                                                                                {{ old('scores.' . $material->id, $score) == $opt ? 'checked' : '' }}
                        ...
                        <div class="space-y-6 mb-8">
                            <div class="border-b-2 border-indigo-500 pb-2">
                                ...
                                <p class="text-xs text-gray-500 mt-1">Gunakan tombol 'Generate dari Checklist' untuk
                                    membuat draft narasi berdasarkan nilai di atas.</p>
                            </div>
                            ...
                            @foreach ($themes as $theme)
                                <div class="bg-gray-50 rounded-xl border border-gray-200 p-5">
                                    ...
                                    <textarea name="theme_notes[{{ $theme->id }}]" id="theme_note_{{ $theme->id }}" rows="4"
                                        ...
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 bg-white leading-relaxed"
                                        placeholder="Narasi untuk tema {{ $theme->theme_name }}...">{{ old('theme_notes.' . $theme->id, $savedThemeNotes[$theme->id] ?? '') }}</textarea>
                                </div>
                            @endforeach
                        </div>
                            ...
                            @php
                                $sections = [
                                    [
                                        ...
                                        'photo' => 'foto_agama',
                                        'db_col' => 'religious_values_text',
                                        'old_photo' => $report->religious_values_photo,
                                    ],
                                    [
                                        'id' => 'narasi_jatidiri',
                                        'title' => 'Jati Diri',
                                        ...
                                        'color' => 'text-blue-500',
                                        'photo' => 'foto_jatidiri',
                                        'db_col' => 'identity_text',
                                        'old_photo' => $report->identity_photo,
                                    ],
                                    [
                                        'id' => 'narasi_steam',
                                        ...
                                        'photo' => 'foto_steam',
                                        'db_col' => 'literacy_steam_text',
                                        'old_photo' => $report->literacy_steam_photo,
                                    ],
                                    [
                                        'id' => 'narasi_p5',
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div>
                                                <textarea name="{{ $sec['id'] }}" id="{{ $sec['id'] }}" rows="5" oninput="autoResize(this)"
                                                    ...
                                                    placeholder="Narasi umum aspek ini...">{{ old($sec['db_col'], $report->{$sec['db_col']}) }}</textarea>
                                            </div>
                                            <div
                                                ...
                                                @if ($sec['old_photo'])
                                                    <div id="preview_container_{{ $sec['photo'] }}"
                                                        class="relative file-preview-container">
                                                        <img src="{{ asset('storage/' . $sec['old_photo']) }}"
                                                            id="img_prev_{{ $sec['photo'] }}" alt="Preview"
                                                        ...
                                                        <input type="hidden" name="delete_photo_{{ $sec['photo'] }}"
                                                            id="delete_photo_{{ $sec['photo'] }}" value="0">
                                                    </div>
                                                    <div id="upload_box_{{ $sec['photo'] }}" class="hidden mt-2">
                                                ...
                                                @else
                                                    <div id="upload_box_{{ $sec['photo'] }}">
                                                        ...
                                                        <p class="text-[10px] text-gray-400 mt-2">JPG/PNG Max 5MB</p>
                                                    </div>
                                                    <div id="preview_container_{{ $sec['photo'] }}"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <textarea name="teacher_notes" rows="4" oninput="autoResize(this)"
                                        ...
                                        placeholder="Contoh: Ananda sangat antusias dalam kegiatan seni, namun perlu dorongan lebih saat kegiatan fisik...">{{ old('teacher_notes', $report->teacher_notes) }}</textarea>
                                </div>
                                <div>
                                    ...
                                    <textarea name="recommendations" rows="4" oninput="autoResize(this)"
                                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 bg-white"
                                        placeholder="Contoh: Mohon Ayah/Bunda melatih kemandirian ananda...">{{ old('recommendations', $report->recommendations) }}</textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="hidden p-6 rounded-lg bg-white border border-gray-200" id="health"
                        role="tabpanel">
                        ...
                        <div class="mb-8">
                            @if ($report->weight && $report->height)
                                <div class="bg-green-50 border-l-4 border-green-500 p-5 rounded-r-lg shadow-sm">
                                    <div class="flex">
                                        <i class="fas fa-info-circle text-green-500 text-xl flex-shrink-0"></i>
                                        <div class="ml-4 w-full">
                                            <h3 class="text-sm font-bold text-green-800 uppercase tracking-wide">Data
                                                Fisik Tersimpan pada Raport ini</h3>
                                            <div class="mt-2 grid grid-cols-3 gap-3 text-xs">
                                                <div class="bg-white p-2 rounded shadow-sm">
                                                    <span class="text-gray-500 block font-semibold">BB</span>
                                                    <span
                                                        class="font-bold text-green-700 text-sm">{{ $report->weight }}
                                                        kg</span>
                                                </div>
                                                <div class="bg-white p-2 rounded shadow-sm">
                                                    <span class="text-gray-500 block font-semibold">TB</span>
                                                    <span
                                                        class="font-bold text-green-700 text-sm">{{ $report->height }}
                                                        cm</span>
                                                </div>
                                                <div class="bg-white p-2 rounded shadow-sm">
                                                    <span class="text-gray-500 block font-semibold">LK</span>
                                                    <span
                                                        class="font-bold text-green-700 text-sm">{{ $lk_value ?? '-' }}
                                                        cm</span>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            @else
                                <div class="bg-yellow-50 border-l-4 border-yellow-400 p-5 rounded-r-lg shadow-sm">
                                    <div class="flex">
                                        ...
                                        <div class="ml-4">
                                            <h3 class="text-sm font-bold text-yellow-800 uppercase tracking-wide">Data
                                                Fisik Kosong</h3>
                                            <p class="text-sm text-yellow-700 mt-1">Silakan isi manual di bawah ini
                                                jika ingin mengubah atau menambahkan data.</p>
                                        </div>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
                                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                                    <h3 class="font-bold text-gray-800"><i
                                            class="fas fa-weight mr-2 text-indigo-500"></i>Edit Data Pertumbuhan</h3>
                                </div>
                                <div class="p-6">
                                    <div class="grid grid-cols-3 gap-4 mb-6">
                                        <div>
                                            ...
                                            <input type="number" step="0.1" name="berat_badan" id="berat_badan"
                                                value="{{ old('berat_badan', $report->weight) }}"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 bg-gray-50 font-bold text-center">
                                        </div>
                                        <div>
                                            <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Tinggi
                                                (cm)</label>
                                            <input type="number" step="0.1" name="tinggi_badan"
                                                id="tinggi_badan" value="{{ old('tinggi_badan', $report->height) }}"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 bg-gray-50 font-bold text-center">
                                        </div>
                                        <div>
                                            ...
                                            <input type="number" step="0.1" name="lingkar_kepala"
                                                id="lingkar_kepala" value="{{ old('lingkar_kepala', $lk_value) }}"
                                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 bg-gray-50 font-bold text-center">
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-4">
                                        <div class="flex justify-between items-center mb-2">
                                            ...
                                            <button type="button" onclick="generateConclusionAI()"
                                                class="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded hover:bg-indigo-100 transition border border-indigo-200">
                                        ...
                                        <textarea name="info_perkembangan" id="info_perkembangan" rows="6" oninput="autoResize(this)"
                                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3"
                                            placeholder="Tuliskan kesimpulan pertumbuhan fisik di sini...">{{ old('info_perkembangan', $report->development_info_text) }}</textarea>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-bold text-gray-700 mb-2">Upload Grafik
                                            KMS</label>
                                        <div id="upload_box_development_info_photo"
                                            class="bg-gray-50 p-4 rounded-lg border border-dashed border-gray-300 text-center {{ $report->development_info_photo ? 'hidden' : '' }}">
                                            <input type="file" name="development_info_photo"
                                                id="development_info_photo" accept="image/*"
                                                onchange="handleFilePreview(this, 'preview_container_kms', 'upload_box_development_info_photo')"
                                                class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 cursor-pointer" />
                                        </div>
                                        <div id="preview_container_kms"
                                            class="relative file-preview-container mt-2 {{ $report->development_info_photo ? '' : 'hidden' }}">
                                            <img src="{{ $report->development_info_photo ? asset('storage/' . $report->development_info_photo) : '' }}"
                                                id="img_prev_development_info_photo" alt="Preview KMS"
                                                class="w-full h-auto rounded shadow-sm border border-gray-200">
                                            <button type="button"
                                                onclick="removeFile('development_info_photo', 'preview_container_kms', 'upload_box_development_info_photo')"
                                                class="absolute top-2 right-2 bg-white text-red-600 p-2 rounded-full shadow hover:bg-gray-100 border border-gray-200">
                                                <i class="fas fa-trash-alt"></i>
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <div class="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden h-fit">
                                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                                    <h3 class="font-bold text-gray-800"><i
                                            class="fas fa-notes-medical mr-2 text-red-500"></i>Pemeriksaan Kesehatan
                                    </h3>
                                </div>
                                <div class="p-6 space-y-3">
                                    @foreach ($healthItems as $index => $item)
                                        <div
                                            class="flex items-center justify-between p-2 rounded hover:bg-gray-50 border border-transparent hover:border-gray-100 transition">
                                            <span class="text-sm font-medium text-gray-700">{{ $item }}</span>
                                            <select name="health[{{ $item }}]"
                                                id="health_{{ $index }}"
                                                class="health-select block w-40 pl-3 pr-8 py-1.5 text-xs border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md shadow-sm">
                                                <option value="Baik"
                                                    {{ old('health.' . $item, $healthDetails[$item] ?? '') == 'Baik' ? 'selected' : '' }}>
                                                    Baik</option>
                                                <option value="Cukup"
                                                    {{ old('health.' . $item, $healthDetails[$item] ?? '') == 'Cukup' ? 'selected' : '' }}>
                                                    Cukup</option>
                                                <option value="Perlu Perhatian"
                                                    {{ old('health.' . $item, $healthDetails[$item] ?? '') == 'Perlu Perhatian' ? 'selected' : '' }}>
                                                    Perlu Perhatian</option>
                                                <option value="Perlu Tindakan"
                                                    {{ old('health.' . $item, $healthDetails[$item] ?? '') == 'Perlu Tindakan Medis' ? 'selected' : '' }}>
                                                    Perlu Tindakan</option>
                                            </select>
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    <div class="hidden p-6 rounded-lg bg-white border border-gray-200" id="legal"
                        role="tabpanel">
                        <div class="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200">
                            <div class="bg-gray-800 px-6 py-4 text-center">
                                <h3 class="text-lg font-bold text-white uppercase tracking-wider">Lembar Pengesahan
                                    Digital</h3>
                            </div>
                            <div class="p-8 bg-gray-50">
                                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                    @php
                                        $signatures = [
                                            [
                                                ...
                                                'default_name' => $report->parent_name ?? '',
                                                'sig_file' => $report->parent_signature,
                                            ],
                                            [
                                                'id' => 'sig_guru',
                                                'input' => 'ttd_guru',
                                                ...
                                                'label' => 'Wali Kelas',
                                                'default_name' => $report->teacher_name ?? $defaultTeacherName,
                                                'sig_file' => $report->teacher_signature,
                                            ],
                                            [
                                                'id' => 'sig_konsultan',
                                                'input' => 'ttd_konsultan',
                                                ...
                                                'label' => 'Konsultan',
                                                'default_name' => $report->consultant_name ?? $defaultConsultantName,
                                                'sig_file' => $report->consultant_signature,
                                            ],
                                            [
                                                'id' => 'sig_kepsek',
                                                'input' => 'ttd_kepsek',
                                                ...
                                                'default_name' => $report->principal_name ?? $defaultPrincipalName,
                                                'sig_file' => $report->principal_signature,
                                            ],
                                        ];
                                    @endphp
                                    ...
                                    @foreach ($signatures as $sig)
                                        <div
                                            class="flex flex-col items-center bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                                            <label
                                                class="block text-xs font-bold text-gray-500 mb-3 uppercase tracking-wide">{{ $sig['label'] }}</label>
                                            <div
                                                class="relative group w-full h-40 border-2 border-dashed border-gray-300 rounded-lg bg-white hover:border-indigo-500 transition-colors">
                                                <canvas id="{{ $sig['id'] }}"
                                                    class="w-full h-full cursor-crosshair"></canvas>

                                                @if ($sig['sig_file'] && Storage::disk('public')->exists($sig['sig_file']))
                                                    <img src="{{ asset('storage/' . $sig['sig_file']) }}"
                                                        id="img_sig_prev_{{ $sig['id'] }}"
                                                        class="absolute inset-0 w-full h-full object-contain p-2 pointer-events-none"
                                                        alt="Tanda Tangan Lama">
                                                @endif

                                                <button type="button"
                                                    class="absolute top-2 right-2 text-gray-400 hover:text-red-600 bg-white rounded-full p-1.5 shadow-sm opacity-0 group-hover:opacity-100 transition-all duration-200 clear-sig-btn border border-gray-200"
                                                    data-target="{{ $sig['id'] }}" title="Hapus Tanda Tangan">
                                                    <i class="fas fa-trash-alt"></i>
                                                </button>
                                                <span
                                                    class="absolute bottom-2 left-2 text-[10px] text-gray-300 pointer-events-none select-none">Area
                                                    Tanda Tangan</span>
                                            </div>
                                            ...
                                            <div class="w-full mt-4">
                                                <label
                                                    class="block text-[10px] text-gray-400 mb-1 ml-1 uppercase font-bold">Nama
                                                    Terang</label>
                                                <input type="text" name="{{ $sig['name_input'] }}"
                                                    value="{{ old($sig['name_input'], $sig['default_name']) }}"
                                                    placeholder="{{ $sig['label'] }}"
                                                    class="text-center w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm font-semibold p-2">
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="mt-8 flex justify-end space-x-4 pb-8">
                            <button type="button"
                                onclick="confirmBack('{{ route('reports.history', $report->student_id) }}')"
                                class="bg-white py-3 px-6 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 transition">
                                Batal
                            </button>
                            <button type="submit" onclick="submitForm(event)"
                                class="inline-flex justify-center py-3 px-8 border border-transparent shadow-lg text-sm font-bold rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all transform hover:scale-105">
    ...
    <script>
        ...
        function removeFile(inputId, previewContainerId, uploadBoxId) {
            Swal.fire({
                title: 'Hapus foto?',
                text: "Foto yang dipilih akan dihapus. (Ini akan menghapus foto lama jika ada)",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    // Cek jika ada image preview lama dari database
                    const imgOld = document.getElementById('img_sig_prev_' + inputId);
        ...
        // --- 3. GENERATE DRAFT PER TEMA (RULE BASED) ---
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelector('#profile-tab').classList.add('text-indigo-600', 'border-indigo-600');
            ...
            tabs.forEach(tab => {
                tab.addEventListener('click', () => {
                    ...
                    if (tab.id === 'legal-tab') {
                        // Inisialisasi/Resize canvas saat tab signature dibuka
                        canvasIds.forEach(id => {
                            const canvas = document.getElementById(id);
                            const pad = pads[id];
                            if (pad) pad.clear();
                            if (canvas.offsetWidth > 0) resizeCanvas(canvas);
                            const imgOld = document.getElementById('img_sig_prev_' + id);
                            if (imgOld) imgOld.style.display = 'block';
                        });
                    }
                });
            });
        ...
        function resizeAllCanvases() {
            canvasIds.forEach(id => {
                const canvas = document.getElementById(id);
                if (canvas.offsetWidth > 0 && pads[id]) {
                    resizeCanvas(canvas);
                    pads[id].clear();
                }
            });
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.querySelectorAll('.clear-sig-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    const imgOld = document.getElementById('img_sig_prev_' + targetId);
                    if (imgOld) imgOld.style.display = 'none';

                    document.getElementById('input_' + targetId).value = '';
                });
            });
        });
