<x-app-layout>
    ...
    @php
        ...
        // Helper untuk menentukan Status Usia (Bucket)
        // Termasuk logika baru untuk rentang 75% - 100%
        function getAgeBucketConfig($age, $p25, $p75, $p100)
        {
            $p25 = (int) $p25;
            $p75 = (int) $p75;
            $p100 = (int) $p100;
            ...
            if ($age < $p25) {
                return ['label' => 'Belum Waktunya', 'class' => 'bg-gray-200 text-gray-700'];
            }
            if ($age > $p100) {
                return ['label' => 'Lewat Usia', 'class' => 'bg-red-100 text-red-700'];
            }
            // Rentang Kritis 75% - 100%
            if ($age >= $p75 && $age <= $p100) {
                return [
                    'label' => 'Zona Kritis (75-100%)',
                    'class' => 'bg-orange-100 text-orange-800 border border-orange-200 font-bold',
                ];
            }
            // Rentang Normal (P25 - P75)
            if ($age >= $p25 && $age < $p75) {
                return ['label' => 'Rentang Usia', 'class' => 'bg-blue-100 text-blue-700'];
            }

            return ['label' => 'Di Garis Usia', 'class' => 'bg-blue-100 text-blue-700'];
        }
    @endphp
    ...
    <form id="assessment-form" action="{{ route('mmdst-assessments.store') }}" method="POST" class="space-y-6">
        ...
        <div class="p-6 bg-white dark:bg-gray-900 rounded-md shadow">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6"> {{-- Grid responsive --}}
        ...
        <div class="p-4 bg-white dark:bg-gray-900 rounded-md shadow text-[11px] md:text-xs">
            <div class="space-y-2">
                <div class="flex flex-wrap gap-2 md:gap-3 items-center">
                    ...
                    <span class="px-2 py-0.5 rounded bg-green-100 text-green-700 border border-green-200">LULUS
                        (P)</span>
                    <span class="px-2 py-0.5 rounded bg-red-100 text-red-700 border border-red-200">GAGAL (F)</span>
                    <span class="px-2 py-0.5 rounded bg-yellow-100 text-yellow-700 border border-yellow-200">ULANG
                        (R)</span>
                    <span class="px-2 py-0.5 rounded bg-gray-200 text-gray-700 border border-gray-300">BELUM (OP)</span>
                </div>
                <div class="flex flex-wrap gap-2 md:gap-3 items-center">
                    <span class="font-medium">Status Usia:</span>
                    <span class="px-2 py-0.5 rounded bg-blue-100 text-blue-700">Rentang Usia (25-75%)</span>
                    <span
                        class="px-2 py-0.5 rounded bg-orange-100 text-orange-800 font-bold border border-orange-300">Zona
                        Kritis (75-100%)</span>
                    <span class="px-2 py-0.5 rounded bg-red-100 text-red-700">Lewat Usia (>100%)</span>
                    <span class="px-2 py-0.5 rounded bg-gray-200 text-gray-700">Belum Waktunya</span>
                </div>
                <ul class="list-disc pl-5 text-gray-600 italic">
                    <li><b>Zona Kritis (75-100%)</b>: Jika anak Gagal (F) pada item di rentang ini, item tersebut
                        dianggap sebagai keterlambatan (Delay).</li>
                    <li>Bila <b>Lewat Usia</b> namun hasil <b>LULUS</b>, tetap dianggap sesuai (normal).</li>
                </ul>
            </div>
        </div>
                        ...
                        <table class="min-w-full table-fixed text-sm">
                            <thead class="bg-white dark:bg-gray-800/80 text-gray-600 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th class="py-2 px-3 text-left hidden md:table-cell w-[24rem]">Deskripsi</th>
                                    {{-- Kolom NR dihapus --}}
                                    ...
                                    <th class="py-2 px-3 text-left w-44">Status Usia</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse ($params as $p)
                                    @php
                                        ...
                                        // Logika Bucket Baru (75-100%)
                                        $bucketConfig = getAgeBucketConfig(
                                            $ageInDays,
                                            $p->percent_25,
                                            $p->percent_75,
                                            $p->percent_100,
                                        ...
                                        $bucketClass = $bucketConfig['class'];
                                    ...
                                    <tr class="border-t border-gray-200 dark:border-gray-700 param-row"
                                        ...
                                        <td class="py-2 px-3 align-top font-medium">
                                            <div class="space-y-1">
                                                ...
                                                <div class="text-[10px] text-gray-500 font-mono">
                                                    %25: {{ $p->percent_25 }} | %75: <b>{{ $p->percent_75 }}</b> |
                                                    %100: {{ $p->percent_100 }}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        @foreach (['P', 'F', 'R', 'OP'] as $opt)
                                            <td class="py-2 px-3 text-center align-top">
                                                <input type="radio" name="items[{{ $rowIndex }}][result_code]"
                                                    ...
                                                    @checked($code === $opt) class="cursor-pointer">
                                            </td>
                                        @endforeach
                                        ...
                                        {{-- Badges: usia + hasil --}}
                                        <td class="py-2 px-3 text-[11px] md:text-xs align-top">
                                            <div class="flex flex-col gap-1 items-start">
                                                <span class="px-2 py-0.5 rounded bucket-badge {{ $bucketClass }}">
                                                    {{ $bucketLabel }}
                                                </span>
                                                <span class="px-2 py-0.5 rounded result-badge {{ $resultBadge }}">
                                                    {{ $resultLabel }}
                                                </span>
                                            </div>
                                        </td>
    ...
    <script>
        ...
        function buildMainNotes() {
            ...
            $$('.param-row').forEach(row => {
                ...
                // Hanya masukkan ke summary jika ada catatan atau hasilnya F/R (Signifikan)
                if (!note && code === 'P' && code === 'OP') return;
                ...
                let line = `- ${paramName} [${code}]`;
                if (note) line += `: ${note}`;

                blocksByCategory[catName].push(line);
            });

            // Gabung jadi teks
            const parts = [];
            Object.keys(blocksByCategory).forEach(cat => {
                if (blocksByCategory[cat].length > 0) {
        ...
        function renderCategories(filteredCategories, previousState, ageInDays) {
            paramContainer.innerHTML = '';
            let rowIndex = 0;
            const age = parseInt(ageInDays || '0', 10) || 0;

            const getBucketInfo = (p25, p75, p100) => {
                if (age < p25) return {
                    cls: 'bg-gray-200 text-gray-700',
                    txt: 'Belum Waktunya'
                };
            ...
            Object.keys(filteredCategories).forEach(catName => {
                ...
                items.forEach(p => {
                    ...
                    const bucket = getBucketInfo(p.percent_25, p.percent_75, p.percent_100);

                    const passed = code === 'P';
                    let resultClass = 'bg-gray-200 text-gray-700';
                    let resultText = '—';
                    if (code === 'P') {
                        resultText = 'LULUS';
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="py-2 px-3 align-top font-medium">
                            <div class="space-y-1">
                                ...
                                <div class="text-[10px] text-gray-500 font-mono">
                                    %25: ${p.percent_25 ?? '-'} | %75: <b>${p.percent_75 ?? '-'}</b> | %100: ${p.percent_100 ?? '-'}
                                </div>
                            </div>
                        </td>
                        ...
                        ${['P','F','R','OP'].map(opt => `
                                    <td class="py-2 px-3 text-center align-top">
                                        <input type="radio" name="items[${rowIndex}][result_code]" value="${opt}" ${enabled ? '' : 'disabled'} ${code===opt?'checked':''} class="cursor-pointer">
                                    </td>`).join('')}
                        ...
                        <td class="py-2 px-3 text-[11px] md:text-xs align-top">
                            <div class="flex flex-col gap-1 items-start">
                                <span class="px-2 py-0.5 rounded bucket-badge ${bucket.cls}">${bucket.txt}</span>
                                <span class="px-2 py-0.5 rounded result-badge ${resultClass}">${resultText}</span>
                            </div>
                        </td>
                    `;
        ...
        function confirmSubmit() {
            ...
            if (invalid) {
                Swal.fire({
                    ...
                    text: 'Pilih P/F/R/OP untuk item yang dicentang.'
                });
                return;
            }
            Swal.fire({
