<x-app-layout>
    ...
    <script>
        ...
        async function loadData() {
            // gunakan route('mmdst.search') relatif (tanpa host) lalu buat URL absolut agar aman
        ...
        /** Tombol "Buat Laporan":
         *  - POST ke /mmdst/{student}/auto-report (startReport)
         *  - Backend mengembalikan { create_url } → redirect ke CREATE
         */
        async function startReport(studentId) {
            ...
            try {
                ...
                if (!res.ok || json.ok === false) throw new Error(json.message || 'Gagal menyiapkan laporan.');
                Swal.fire({
                    icon: 'success',
                    title: 'Siap',
                    text: 'Mengalihkan ke halaman tambah penilaian…',
                    timer: 1000,
                    showConfirmButton: false
                });
                setTimeout(() => {
                    window.location.href = json.create_url; // ⬅️ ke halaman CREATE (bukan edit)
                }, 600);
            } catch (e) {
                Swal.fire({
                    icon: 'error',
