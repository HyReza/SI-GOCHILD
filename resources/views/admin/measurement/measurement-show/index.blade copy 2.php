        ...
        <div class="bg-white dark:bg-gray-900 shadow-md sm:rounded-lg p-6">
            ...
            <div class="space-y-6">
                @php
                    $parameters = [
                        'BB/U' => [
                            'label' => 'Berat Badan vs Umur',
                            'desc' => 'Menunjukkan status gizi anak secara umum (underweight/overweight).',
                        ],
                        'PB/U' => [
                            'label' => 'Panjang Badan vs Umur',
                            'desc' => 'Indikator gizi kronis untuk anak < 2 tahun (stunting).',
                        ],
                        'TB/U' => [
                            'label' => 'Tinggi Badan vs Umur',
                            'desc' => 'Indikator gizi kronis untuk anak ≥ 2 tahun (stunting).',
                        ],
                        'BB/PB' => [
                            'label' => 'Berat Badan vs Panjang Badan',
                            'desc' => 'Indikator gizi akut untuk anak < 2 tahun (kurus/gemuk).',
                        ],
                        'TB/BB' => [
                            'label' => 'Berat Badan vs Tinggi Badan',
                            'desc' => 'Indikator gizi akut untuk anak ≥ 2 tahun (kurus/gemuk).',
                        ],
                        'IMT/U' => [
                            'label' => 'Indeks Massa Tubuh vs Umur',
                            'desc' => 'Indikator skrining kelebihan berat badan dan obesitas.',
                        ],
                    ];
                ...
                @forelse ($parameters as $key => $info)
                    @if (isset($calcResults[$key]))
                        @php
                            $status = $calcResults[$key];
                            ...
                            if (Str::contains($status, ['Sangat Kurang', 'Gizi Buruk', 'Sangat Pendek'])) {
                                $status_color_class = 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-300';
                            } elseif (Str::contains($status, ['Kurang', 'Pendek'])) {
                                $status_color_class =
                        ...
                        <div class="p-4 border rounded-lg dark:border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="md:col-span-1">
                                <h4 class="font-bold text-indigo-600 dark:text-indigo-400">{{ $info['label'] }}
                                    ({{ $key }})</h4>
                                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ $info['desc'] }}</p>
                            </div>
                            <div class="md:col-span-2 grid grid-cols-2 gap-4 text-sm">
                                <div>
                                    <strong class="text-gray-500 dark:text-gray-400 block">Status Gizi</strong>
                                    <p><span
                                            class="px-2 py-1 text-xs font-semibold rounded-full {{ $status_color_class }}">{{ $status }}</span>
                                    </p>
                                </div>
                                <div>
                                    <strong class="text-gray-500 dark:text-gray-400 block">Kategori SD</strong>
                                    <p class="text-gray-800 dark:text-gray-200">{{ $sd_cat }}</p>
                                </div>
                                <div>
                                    <strong class="text-gray-500 dark:text-gray-400 block">Nilai Z-Score</strong>
                                    <p class="text-gray-800 dark:text-gray-200 font-mono">{{ $z_score }}</p>
                                </div>
                                @if ($imt_value)
                                    <div>
                                        <strong class="text-gray-500 dark:text-gray-400 block">Nilai IMT</strong>
                                        <p class="text-gray-800 dark:text-gray-200 font-mono">{{ $imt_value }}</p>
                                    </div>
                                @endif
                            </div>
                        </div>
