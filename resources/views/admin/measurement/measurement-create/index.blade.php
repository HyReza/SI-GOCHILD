<x-app-layout>
    <x-slot:title>Form Pengukuran Anak</x-slot:title>
                ...
                <div class="sm:col-span-2">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <p id="age-display" class="text-gray-800 dark:text-gray-200 font-medium text-base">
                                Menghitung...</p>
                        </div>
                        <div>
                            ...
                            <p class="text-gray-800 dark:text-gray-200 text-sm">
                                ...
                                {{ $activityTransaction->student->subdistrict }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-900 shadow-md sm:rounded-lg p-6">
            <form id="measurementForm" action="{{ route('measurement.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Posisi Saat
                            Diukur</label>
                        <div class="mt-2 flex space-x-4">
                            <label class="inline-flex items-center"><input type="radio" name="measurement_condition"
                                    value="berdiri" id="condition_berdiri"
                                    class="form-radio text-indigo-600 dark:bg-gray-800 dark:border-gray-600"><span
                                    class="ml-2 text-gray-700 dark:text-gray-300">Berdiri</span></label>
                            <label class="inline-flex items-center"><input type="radio" name="measurement_condition"
                                    value="terlentang" id="condition_terlentang"
                                    class="form-radio text-indigo-600 dark:bg-gray-800 dark:border-gray-600"><span
                                    class="ml-2 text-gray-700 dark:text-gray-300">Terlentang</span></label>
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="height" class="block text-sm font-medium text-gray-700 dark:text-gray-300"
                            id="height_label">Panjang/Tinggi Badan (cm)</label>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function fetchGrowthStandards() {
                ...
                const actualCondition = conditionEl.value;
                ...
                if (ageInMonths < 24) {
                    finalCondition = 'terlentang';
                    document.getElementById('height_label').textContent = 'Panjang Badan (PB) (cm)';
                    if (actualCondition === 'berdiri') {
                        finalHeight = height + 0.7;
                        note =
                            `Koreksi: Anak < 24 bulan diukur berdiri. Hasil dikonversi ke Panjang Badan (tambah 0.7 cm): ${height} cm → ${finalHeight.toFixed(2)} cm.`;
                    }
                } else {
                    finalCondition = 'berdiri';
                    document.getElementById('height_label').textContent = 'Tinggi Badan (TB) (cm)';
                    if (actualCondition === 'terlentang') {
                        finalHeight = height - 0.7;
                        note =
                            `Koreksi: Anak ≥ 24 bulan diukur terlentang. Hasil dikonversi ke Tinggi Badan (kurang 0.7 cm): ${height} cm → ${finalHeight.toFixed(2)} cm.`;
                    }
                }

                if (note) {
                    adjustmentNote.textContent = note;
                ...
                try {
                    const apiUrl =
                        `{{ route('api.growth-standards') }}?gender=${studentGender}&age_months=${ageInMonths}&height=${finalHeight}&measurement_condition=${finalCondition}`;
                    const response = await fetch(apiUrl);
                    if (!response.ok) {
                        throw new Error('Gagal mengambil data standar.');
                    }
                    ...
                    calculateAndDisplayResults(data, weight, finalHeight, finalCondition);
                } catch (error) {
            ...
            function calculateAndDisplayResults(standards, weight, height, condition) {
                ...
                const weightHeightParam = standards['PB/BB'] ? 'PB/BB' : (standards['TB/BB'] ? 'TB/BB' : null);
                const bmi = calculateBMI(weight, height);

                document.getElementById('height_param_label').textContent = heightParam === 'TB/U' ?
                    'Tinggi Badan vs Umur (TB/U)' : 'Panjang Badan vs Umur (PB/U)';
                document.getElementById('weight_height_param_label').textContent = weightHeightParam === 'TB/BB' ?
            ...
            function calculateForParameter(standard, value) {
                ...
                let zScore = (value >= median) ? (value - median) / (sd_plus_1 - median) : (value - median) / (
                    median - sd_minus_1);
                const zScoreFixed = zScore.toFixed(2);
                return {
                    zScore: zScoreFixed,
            ...
            function getStatus(parameter, zScore) {
                ...
                if (parameter === 'BB/PB' || parameter === 'PB/BB') mainParam = 'BB/TB';
                if (parameter === 'TB/BB') mainParam = 'BB/TB';
                switch (mainParam) {
                    case 'BB/U':
            ...
            [weightInput, heightInput, ...document.querySelectorAll('input[name="measurement_condition"]')].forEach(
                el => el.addEventListener('input', fetchGrowthStandards));
