        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function fetchGrowthStandards() {
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error('Gagal mengambil data standar dari server.');
                    }
            ...
            function calculateAndDisplayResults(standards, weight, height, condition) {
                ...
                const weightHeightParam = condition === 'berdiri' ? 'BB/TB' : 'BB/PB';
                ...
                document.getElementById('weight_height_param_label').textContent = weightHeightParam === 'BB/TB' ?
                    'Berat Badan vs Tinggi Badan (BB/TB)' : 'Berat Badan vs Panjang Badan (BB/PB)';
                ...
                // PERBAIKAN: Gunakan nama parameter yang benar dari data (PB/BB atau TB/BB)
                const realWeightHeightParam = standards['BB/PB'] ? 'BB/PB' : (standards['BB/TB'] ? 'BB/TB' : null);
                if (realWeightHeightParam) {
                    results[realWeightHeightParam] = calculateForParameter(standards[realWeightHeightParam],
                    weight);
                    updateUI('bbtb', results[realWeightHeightParam]);
                } else {
                    updateUI('bbtb', {
                        zScore: 'N/A',
                ...
                fillHiddenInputs(results, heightParam, realWeightHeightParam, bmi);
            }
            ...
            function calculateForParameter(standard, value) {
                ...
                return {
                    ...
                    sdCategory: getSDCategory(zScore.toFixed(2)),
                    status: getStatus(standard.parameter, zScore.toFixed(2))
                };
            }
