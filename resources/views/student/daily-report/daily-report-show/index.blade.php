<x-app-layout>
    @push('head')
        ...
        {{-- Library Signature Pad (Wajib untuk Orang Tua Tanda Tangan) --}}
                ...
                <div class="flex flex-col items-center">
                    ...
                    @if ($dailyReport->parent_guardian_signature)
                        ...
                        <p class="mt-3 font-bold text-gray-800 dark:text-white">
                            {{ $dailyReport->parent_guardian_name }}</p>
                        <p class="text-xs text-gray-400">Terkonfirmasi</p>
                    @else
                        {{-- BELUM TTD (INPUT FORM) --}}
                        <div class="w-full max-w-sm">
                            <form id="signature-form"
                                action="{{ route('student.daily-report.sign', $dailyReport->id) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                    <input type="text" name="parent_name" id="parent_name" required
                                        class="w-full rounded-lg border-gray-300 text-sm text-center placeholder-gray-400 focus:ring-pink-500 focus:border-pink-500"
                                        placeholder="Ketik Nama Orang Tua/Wali"
                                        value="{{ Auth::check() && Auth::user()->role == 'parent' ? Auth::user()->name : '' }}">
                                </div>

                                <div class="relative group mb-3">
                                    <canvas id="signature-pad"
                                        class="block w-full bg-gray-50 dark:bg-gray-800 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl cursor-crosshair touch-none shadow-inner h-40"></canvas>
                                    <div
                                ...
                                <div class="flex justify-center gap-2">
                                    <button type="button" id="clear-btn"
                                        class="px-3 py-1.5 text-xs bg-gray-100 border border-gray-300 rounded hover:bg-gray-200 text-gray-700">Ulangi</button>
                                    <button type="button" id="save-btn"
                                        class="px-4 py-1.5 text-xs bg-pink-600 text-white rounded font-bold hover:bg-pink-700 shadow">Konfirmasi</button>
                                </div>
                            </form>
                        </div>
                    @endif
    ...
    <form id="deleteForm" action="{{ route('daily-report.destroy', $dailyReport) }}" method="POST" class="hidden">
    ...
    <script>
        ...
        // 2. Signature Logic (Only if canvas exists)
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (canvas) {
                ...
                document.getElementById('save-btn').addEventListener('click', () => {
                    const nameVal = document.getElementById('parent_name').value.trim();

                    if (!nameVal) {
                        Swal.fire('Nama Wajib Diisi', 'Silakan masukkan nama orang tua/wali.', 'warning');
                        return;
                    }
                    if (signaturePad.isEmpty()) {
                        Swal.fire('Tanda Tangan Kosong', 'Silakan tanda tangan pada kotak yang tersedia.',
                            'warning');
                        return;
                    }
